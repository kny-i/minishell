...
void	assign_token(t_token *token, int *i_token, int input, t_cmd_len ***len)
{
	token->data[*i_token] = input;
	*i_token += 1;
	token->data[*i_token] = '\0';
...
void	token_set(t_token *token, t_cmd_len *len)
{
	while (len != NULL)
	{
		...
		if (token == NULL)
			exit(1);
		token->data = malloc(sizeof(len->len + 1));
		if (token->data == NULL)
			exit(1);
		ft_bzero(token->data, len->len);
		token = token->next;
		len = len->next;
	}
}
